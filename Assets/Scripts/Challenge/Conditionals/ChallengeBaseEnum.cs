...
public abstract class ChallengeBaseEnum<T> : ChallengeBase
where T : System.Enum{
	public ChallengeBaseEnum() {
		tests = new TestItem[] {
			new TestItem(
				"42 + 6",
				new TestField[] {
	...
	internal override bool RunTest(TestItem p_test) {
		CallFun<int,int,T>(p_test, EnumConditional);
		return Result.CheckSame(p_test.expectedResults);
	}
	protected virtual void EnumConditional(int p_a, int p_b, T p_operator) { throw new NotDoException(); }
}
